package ruolan.com.livestreaming.logic;
...
import ruolan.com.livestreaming.utils.LogDebugUtil;
...
public class IMLogin {
    ...
    private static IMLogin mInstance;
    ...
    //登录接口回调
    private IMLoginLinstener mIMLoginLinstener;
     ...
     * 登录接口回调
     */
    public interface IMLoginLinstener {
        /**
         * 成功回调
         */
        void onSuccess();
    ...
    public void setIMLoginLinstener(IMLoginLinstener IMLoginLinstener) {
        mIMLoginLinstener = IMLoginLinstener;
    }
    ...
    public void pwdLogin(String username, String password) {
        ...
        if (!TextUtils.isEmpty(username) && !TextUtils.isEmpty(password)) {
            request = new LoginRequest(RequestComm.register, username, password);
        }
        AsyncHttp.instance().postJson(request, new AsyncHttp.IHttpListener() {
            ...
            public void onSuccess(int requestId, Response response) {
                ...
                if (response.status == RequestComm.SUCCESS) {
                    ...
                    if (null != mIMLoginLinstener) {
                        mIMLoginLinstener.onFailure(0, "登陆失败:" + response.msg);
                    }
                }
            }
            ...
            public void onFailure(int requestId, int httpStatus, Throwable error) {
                if (null != mIMLoginLinstener) {
                    mIMLoginLinstener.onFailure(httpStatus, error.getMessage());
                }
            }
        });
    }
    ...
    public void imLogin(@NonNull String identify, @NonNull String userSig) {
        ...
        TIMManager.getInstance().login(Constants.IMSDK_APPID, user, userSig, new TIMCallBack() {
            ...
            public void onError(int i, String s) {
                ...
                if (null != mIMLoginLinstener)
                    mIMLoginLinstener.onFailure(i, s);
            }
    ...
    private TLSSmsLoginListener mTLSSmsLoginListener = new TLSSmsLoginListener() {
        ...
        public void OnSmsLoginFail(TLSErrInfo tlsErrInfo) {
            if (null != mIMLoginLinstener)
                mIMLoginLinstener.onFailure(tlsErrInfo.ErrCode, tlsErrInfo.Msg);
        }
